...
public:
    ...
    void set_blocks(std::vector<std::string> const &text, float width);
    // single lines use a baseline origin and wrapped blocks use a top-left origin
    void draw(glm::uvec2 logical_viewport, glm::vec2 origin, glm::vec4 color, size_t block_index = 0);
    ...
    float advance() const { return blocks.empty() ? 0 : blocks.front().width; }
    TextBlock const &text_block(size_t index = 0) const { return blocks.at(index); }
private:
    struct Glyph {
    ...
    struct Batch { size_t page, block; GLint first; GLsizei count; };
    Glyph const &ensure_glyph(uint32_t id);
    void add_page();
    void upload(std::vector<TextBlock> const &prepared);
    void release();
    FontAsset &font;
    TextLayout layout;
    TextProgram shader;
